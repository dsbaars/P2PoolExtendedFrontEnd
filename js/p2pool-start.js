			...
			function LoadData()
			{
				$("#blocks").html("<thead><tr><th>ID</th><th>Time</th><th>Hash / Explorer link</th><th>Share</th></tr></thead><tbody></tbody>");
				$("#payouts").html("<thead><tr><th>address</th><th>amount in <span class=\"symbol\"></span></th></tr></thead><tbody></tbody>");
				...
				d3.json('/current_payouts', function(pays) {
				    ...
				    var tr = d3.select('#payouts > tbody').selectAll().data(arr).enter().append('tr');
				    ...
				    var total_tr = d3.select('#payouts > tbody').append('tr');
				...
				d3.json('/recent_blocks', function(blocks) {
				    var tr = d3.select('#blocks > tbody').selectAll().data(blocks).enter().append('tr');
				    ...
				    parseDate = d3.time.format("%H:%M %d-%m-%Y");

				    blocksummary = blocks.slice(0,8);

				    var tr = d3.select('#miniblocks > tbody').selectAll().data(blocksummary).enter().append('tr');
				    tr.append('td').text(function(block){return block.number});
				    tr.append('td').text(function(block){return parseDate(new Date(1000*block.ts))});
				    tr.append('td').append('a').text("Link").attr('href', function(block){return currency_info.block_explorer_url_prefix + block.hash});
				    tr.append('td').append('a').text('→').attr('href', function(block){return 'share.html#' + block.share});
				});
			    });
			...
			function UpdateData()
			{
				...
				d3.json('/local_stats', function(local_stats) {
					...
					d3.json('/global_stats', function(global_stats) {
						...
						d3.select('#time_to_block_mins').text(d3.format('.3r')(time_to_block/216) + " mins");
